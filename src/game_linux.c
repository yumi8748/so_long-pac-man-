...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/03/13 17:05:02 by yu-chen           #+#    #+#             */
/*   Updated: 2024/05/08 15:27:59 by yu-chen          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	key_hook(int key, t_game *g)
{
	...
	if (key == KEY_R && !g->pac_dying)
		ft_reset(g);
	if ((key == KEY_UP || key == KEY_W) && g->next_dir != N && !g->pac_dying)
		ft_newdirection(g, N);
	if ((key == KEY_DOWN || key == KEY_S) && g->next_dir != S && !g->pac_dying)
		ft_newdirection(g, S);
	if ((key == KEY_RIGHT || key == KEY_D) && g->next_dir != E && !g->pac_dying)
		ft_newdirection(g, E);
	if ((key == KEY_LEFT || key == KEY_A) && g->next_dir != W && !g->pac_dying)
		ft_newdirection(g, W);
	return (0);
}
...
int	init_game(char **map, t_lay lay)
{
	...
	ft_bzero(&g.sprites, sizeof(g.sprites));
	ft_bzero(&g.sprites.score_font, sizeof(g.sprites.score_font));
	g.lay_bak = lay;
	...
	if (!g.id)
		return (0);
	g.w_id = mlx_new_window(g.id, lay.n_col * SIZE, lay.n_row * SIZE + 80,
			"Pac-Man");
	if (!g.w_id)
	{
		mlx_destroy_display(g.w_id);
		free(g.w_id);
		return (0);
	}
	if (!ft_newgame(&g, map, &lay))
		return (end_game(&g), 0);
	return (0);
}
