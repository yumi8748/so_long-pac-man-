...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/03/12 15:46:34 by yu-chen           #+#    #+#             */
/*   Updated: 2024/05/03 16:33:30 by yu-chen          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_readlayout(int fd, t_err *map_err, t_lay *lay, char **map_str)
{
	...
	while (1)
	{
		...
		*map_str = ft_strconcat_free(*map_str, line);
		lay->n_row++;
	}
}
...
void	ft_checklayout(char *line, t_err *map_err, t_lay *lay, int is_last)
{
	...
	if (line[0] != '1' || line[lay->n_col - 1] != '1' || (ft_countchar(line,
				'1') != lay->n_col && is_last))
...
int	ft_print_map_error(t_lay *lay, t_err *map_err, char **map_str)
{
	...
	if ((lay->n_col < 5 && lay->n_row < 3) || (lay->n_col < 3
			&& lay->n_row < 5))
		error_msg_params("Map is too small!", map_str);
	if (lay->n_col > 100 || lay->n_row > 42)
		error_msg_params("Map is too big!", map_str);
	return (0);
}
